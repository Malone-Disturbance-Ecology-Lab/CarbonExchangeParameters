...
message("This function uses the equation:
        ...
        theta is the convexity (curvilinear angle) of the nonrectangular hyperbola (degrees) 
...
priors <-  prior(normal(-0.01, 0.1), nlpar = "a1", lb=-0.2, ub= 0) +
  ...
  prior(normal(25, 25), nlpar = "theta", lb=0, ub= 3)
...
LRC_PARMS <- function( data.frame, iterations, priors.lrc, idx.colname, NEE.colname , PAR.colname ){
  ...
  for ( i in unique(df$idx)){
    ...
    try(model.brms <- brm( bf( equation, a1+ax+r+theta ~ 1, nl=TRUE),
    ...
    try(model.brms.df.theta <- model.brms.df %>% filter( row.names(model.brms.df) == 'theta_Intercept'), silent = F)
