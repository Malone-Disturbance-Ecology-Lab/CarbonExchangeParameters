...
LRC_PARMS <- function( data.frame, iterations, priors.lrc, idx, nee, PAR){
  ...
  equation <- nee ~ ax*(1-exp(-a1*PAR/ax))- r # exponential equation
  
  # where nee     = Pmax*(1-exp(-a*PAR/Pmax))-Rd 
  # Pmax=ax=light-saturated net photosynthetic rate, 
  # Rd=r is the dark respiration rate
  #reference https://docs.google.com/spreadsheets/d/1kfMe3CtMjuceIHEMsne8wnPSclAk2_ak4Mmb0SM_hew/edit?gid=0#gid=0
