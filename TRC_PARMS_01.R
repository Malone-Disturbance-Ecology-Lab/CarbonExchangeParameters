...
TRC_PARMS_01 <- function( data.frame, iterations, priors.trc, idx.colname, Reco.colname, TA.colname, PAR.colname){
  ...
  data.frame$TA <-data.frame$TA #no need for celsius to kelvin
  ...
  try(equation <- Reco ~ Rref * exp(E0 * (1 / (10 + 46.02) - 1 / (TA + 46.02))) , silent =T)
  ...
  for ( i in unique(df$idx)){
    ...
    #                    family = poisson())
    try(model.brms <-  brm( bf(Reco ~ Rref * exp(E0 * (1 / (10 + 46.02) - 1 / (TA + 46.02))),E0+Rref ~ 1, nl=TRUE),
