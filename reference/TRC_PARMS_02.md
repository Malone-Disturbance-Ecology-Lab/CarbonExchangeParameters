...
This function uses the equation: \$\$\text{NEE} \sim r_0 \cdot \exp
\left( \alpha \cdot T + \beta \cdot T^2 \right)\$\$

Where NEE is nighttime NEE, that is, ecosystem respiration (\\\mu\\mol
CO2 m-2 s-1), TA is air temperature (°C), \\r_0\\ is ecosystem
respiration at TA = 0 °C, and alpha and beta are coefficients.
